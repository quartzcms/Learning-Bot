<?php
	...
	if(isset($_POST['question']) && !empty($_POST['question'])) {
		...
		/* Calculus functions */
		$units = array(
			'chiffres','centimetre','metre','millimetre','kilometre','once','millilitre','litre','metre-carre','centimetre-carre',
			'millimetre-carre','kilometre-carre','metre-cubique','centimetre-cubique','millimetre-cubique','kilometre-cubique'
		);
		function all_units($quantity, $units){
			...
			if($units == 'centimetre'){
				...
				$results['millimetre'] = ($quantity / $quantity) * 1000;
				$results['centimetre'] = ($quantity / $quantity) * 100;
				$results['metre'] = $quantity / $quantity;
				$results['kilometre'] = ($quantity / $quantity) / 1000;
				$results['once'] = 'NA';
				$results['millilitre'] = 'NA';
				$results['litre'] = 'NA';
				...
				$results['millimetre-cubique'] = pow((($quantity / $quantity) * 1000), 3);
				$results['centimetre-cubique'] = pow((($quantity / $quantity) * 100), 3);
				$results['metre-cubique'] = pow((($quantity / $quantity)), 3);
				$results['kilometre-cubique'] = pow((($quantity / $quantity) / 1000), 3);
			} elseif($units == 'centimetre-carre') {
				$results['millimetre'] = ($quantity / $quantity) * 10;
				$results['centimetre'] = ($quantity / $quantity);
				$results['metre'] = ($quantity / $quantity) / 100;
				$results['kilometre'] = ($quantity / $quantity) / 1000000;
				$results['once'] = 'NA';
				$results['millilitre'] = 'NA';
				$results['litre'] = 'NA';
				...
				$results['millimetre-cubique'] = pow((($quantity / $quantity) * 10), 3);
				$results['centimetre-cubique'] = pow((($quantity / $quantity)), 3);
				$results['metre-cubique'] = pow((($quantity / $quantity) / 100), 3);
				$results['kilometre-cubique'] = pow((($quantity / $quantity) / 1000000), 3);
			} elseif($units == 'millimetre-carre') {
				$results['millimetre'] = ($quantity / $quantity);
				$results['centimetre'] = ($quantity / $quantity) / 10;
				$results['metre'] = ($quantity / $quantity) / 1000;
				$results['kilometre'] = ($quantity / $quantity) / 1000000;
				$results['once'] = 'NA';
				$results['millilitre'] = 'NA';
				$results['litre'] = 'NA';
				...
				$results['kilometre-carre'] = $quantity / (1 * pow(10, 9));
				$results['millimetre-cubique'] = pow((($quantity / $quantity)), 3);
				$results['centimetre-cubique'] = pow((($quantity / $quantity) / 10), 3);
				$results['metre-cubique'] = pow((($quantity / $quantity) / 1000), 3);
				$results['kilometre-cubique'] = pow((($quantity / $quantity) / 1000000), 3);
			} elseif($units == 'kilometre-carre') {
				$results['millimetre'] = ($quantity / $quantity) * 1000000;
				$results['centimetre'] = ($quantity / $quantity) * 100000;
				$results['metre'] = ($quantity / $quantity) * 1000;
				$results['kilometre'] = ($quantity / $quantity);
				$results['once'] = 'NA';
				$results['millilitre'] = 'NA';
				$results['litre'] = 'NA';
				...
				$results['millimetre-cubique'] = pow((($quantity / $quantity) * 1000000), 3);
				$results['centimetre-cubique'] = pow((($quantity / $quantity) * 100000), 3);
				$results['metre-cubique'] = pow((($quantity / $quantity) * 1000), 3);
				$results['kilometre-cubique'] = pow((($quantity / $quantity)), 3);
			} elseif($units == 'metre-cubique') {
				$results['millimetre'] = (($quantity / $quantity) / $quantity) * 1000;
				$results['centimetre'] = (($quantity / $quantity) / $quantity) * 100;
				$results['metre'] = (($quantity / $quantity) / $quantity);
				$results['kilometre'] = (($quantity / $quantity) / $quantity) / 1000;
				$results['once'] = $quantity * 33814.023;
				$results['millilitre'] = $quantity * 1000000;
				$results['litre'] = $quantity * 1000;
				$results['millimetre-carre'] = pow(((($quantity / $quantity) / $quantity) * 1000), 2);
				$results['centimetre-carre'] = pow(((($quantity / $quantity) / $quantity) * 100), 2);
				$results['metre-carre'] = pow(((($quantity / $quantity) / $quantity)), 2);
				$results['kilometre-carre'] = pow(((($quantity / $quantity) / $quantity) / 1000), 2);
				$results['millimetre-cubique'] = pow(((($quantity / $quantity) / $quantity) * 1000), 3);
				$results['centimetre-cubique'] = pow(((($quantity / $quantity) / $quantity) * 100), 3);
				$results['metre-cubique'] = $quantity;
				$results['kilometre-cubique'] = pow(((($quantity / $quantity) / $quantity) / 1000), 3);
			} elseif($units == 'centimetre-cubique') {
				$results['millimetre'] = (($quantity / $quantity) / $quantity) * 10;
				$results['centimetre'] = (($quantity / $quantity) / $quantity);
				$results['metre'] = (($quantity / $quantity) / $quantity) / 100;
				$results['kilometre'] = (($quantity / $quantity) / $quantity) / 1000000;
				$results['once'] = $quantity / 29.5735;
				$results['millilitre'] = $quantity;
				$results['litre'] = $quantity / 1000;
				$results['millimetre-carre'] = pow(((($quantity / $quantity) / $quantity) * 10), 2);
				$results['centimetre-carre'] = pow(((($quantity / $quantity) / $quantity)), 2);
				$results['metre-carre'] = pow(((($quantity / $quantity) / $quantity) / 100), 2);
				$results['kilometre-carre'] = pow(((($quantity / $quantity) / $quantity) / 1000000), 2);
				$results['millimetre-cubique'] = pow(((($quantity / $quantity) / $quantity) * 10), 3);
				$results['centimetre-cubique'] = $quantity;
				$results['metre-cubique'] = pow(((($quantity / $quantity) / $quantity) / 100), 3);
				$results['kilometre-cubique'] = pow(((($quantity / $quantity) / $quantity) / 1000000), 3);
			} elseif($units == 'millimetre-cubique') {
				$results['millimetre'] = (($quantity / $quantity) / $quantity);
				$results['centimetre'] = (($quantity / $quantity) / $quantity) / 10;
				$results['metre'] = (($quantity / $quantity) / $quantity) / 1000;
				$results['kilometre'] = (($quantity / $quantity) / $quantity) / 1000000;
				$results['once'] = $quantity / 29573.53;
				$results['millilitre'] = $quantity / 1000;
				$results['litre'] = $quantity / 1000000;
				$results['millimetre-carre'] = pow(((($quantity / $quantity) / $quantity)), 2);
				$results['centimetre-carre'] = pow(((($quantity / $quantity) / $quantity) / 10), 2);
				$results['metre-carre'] = pow(((($quantity / $quantity) / $quantity) / 100), 2);
				$results['kilometre-carre'] = pow(((($quantity / $quantity) / $quantity) / 1000000), 2);
				$results['millimetre-cubique'] = $quantity;
				$results['centimetre-cubique'] = pow(((($quantity / $quantity) / $quantity) / 10), 3);
				$results['metre-cubique'] = pow(((($quantity / $quantity) / $quantity) / 100), 3);
				$results['kilometre-cubique'] = pow(((($quantity / $quantity) / $quantity) / 1000000), 3);
			} elseif($units == 'kilometre-cubique') {
				$results['millimetre'] = (($quantity / $quantity) / $quantity) * 1000000;
				$results['centimetre'] = (($quantity / $quantity) / $quantity) * 100000;
				$results['metre'] = (($quantity / $quantity) / $quantity) * 1000;
				$results['kilometre'] = (($quantity / $quantity) / $quantity);
				$results['once'] = $quantity / 29573.53;
				$results['millilitre'] = $quantity / 1000;
				$results['litre'] = $quantity / 1000000;
				$results['millimetre-carre'] = pow(((($quantity / $quantity) / $quantity) * 1000000), 2);
				$results['centimetre-carre'] = pow(((($quantity / $quantity) / $quantity) * 100000), 2);
				$results['metre-carre'] = pow(((($quantity / $quantity) / $quantity) * 1000), 2);
				$results['kilometre-carre'] = pow(((($quantity / $quantity) / $quantity)), 2);
				$results['millimetre-cubique'] = pow(((($quantity / $quantity) / $quantity) * 1000000), 3);
				$results['centimetre-cubique'] = pow(((($quantity / $quantity) / $quantity) * 100000), 3);
				$results['metre-cubique'] = pow(((($quantity / $quantity) / $quantity) * 1000), 3);
						...
						for($j = ((($i + 1) < count($sentence)) ? ($i + 1) : 0); $j < count($sentence); $j++){
							...
							foreach($units as $value2) {
								if($unit_found == $value2){
									$units_measure[$j] = all_units($sentence[$i], $value2);
									break 3;
								}
							}
						}
				...
				for($j = ((($key + 1) < count($sentence)) ? ($key + 1) : 0); $j < count($sentence); $j++){
					...
					if(is_numeric($sentence[$j])){
						for($k = ((($j + 1) < count($sentence)) ? ($j + 1) : 0); $k < count($sentence); $k++){
							...
							foreach($units as $value2) {
								if($unit_found == $value2){
									$units_measure[$k] = all_units($sentence[$j], $value2);
									break 3;
								}
							}
						}
					...
					if(isset($units_measure[$i]) && is_array($units_measure[$i])){
						foreach($units_measure[$i] as $key2 => $value2) {
							if(isset($units_measure[$i][$key2][$key2])){
								$measure_to_do = $key2;
								$number[] = array($key2, $units_measure[$i][$key2][$key2]);
								break 2;
							}
						}
					}
						...
						if(isset($units_measure[$j]) && is_array($units_measure[$j])){
							foreach($units_measure[$j] as $key2 => $value2) {
								if(isset($units_measure[$j][$key2][$measure_to_do]) && !empty($measure_to_do)){
									$number[] = array($measure_to_do, $units_measure[$j][$key2][$measure_to_do]);
									break 2;
								}
							}
						}
